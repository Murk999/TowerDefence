...
namespace TowerDefense
{
    public class TDPlayer : Player
    {
        ...
        public void TryBuild(TowerAsset towerAsset, Transform buildSite) // команда строить 
        {
            ...
            var tower = Instantiate(m_towerPrefab, buildSite.position, Quaternion.identity); // создаем башню передаем ей параметр префаб позици€ и поворот
            tower.GetComponentInChildren<SpriteRenderer>().sprite = towerAsset.sprite; // ищем компонент спрайт рендерер и мен€ем ему на заданный спрайт 
